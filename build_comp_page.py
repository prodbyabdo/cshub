...
# Configuration
SOURCE_DIR = Path("output_pdfplumber/markdown_cache/Comp Applications")
OUTPUT_HTML = "year1sem1compapp.html"
...
<head>
    ...
    <title>Computer Applications | CS Hub</title>
    ...
    <style>
        ...
        .definition {{ display: grid; grid-template-columns: 150px 1fr; gap: 20px; }}
        ...
        /* Matching specifics */
        .matching-pair {{ display: flex; justify-content: space-between; border-bottom: 1px solid #222; padding: 5px 0; }}
    </style>
</head>
<body>
    <div class="container">
        ...
        <header>
            ...
            <div class="subtitle">COM-111 Resources & Study Guide</div>
        </header>
    ...
    for i, line in enumerate(lines):
        ...
        if in_matching_section:
            if line.startswith("**Group"):
                if "table" in html and "Group" not in line: html += "</table>" # Close previous table if exists
                html += f"<h4>{line.replace('**', '')}</h4>"
                # Start a table for the group
                html += '<table style="width:100%; border-collapse: collapse; margin-bottom: 20px;">'
                continue
            
            # Check if it's a matching line "51. Item ....... A. Description"
            match = re.search(r'^(\d+\..*?)(\.{2,})\s*([A-Z]\..*)', line)
            if match:
                # Row
                col1 = match.group(1).strip()
                col2 = match.group(3).strip()
                html += f'<tr><td style="padding: 8px; border-bottom: 1px solid #333; width: 50%;">{col1}</td><td style="padding: 8px; border-bottom: 1px solid #333; width: 50%;">{col2}</td></tr>'
        ...
        # Normal Questions Handling (Part 1, 2, 4)
        if line.startswith("###"):
        ...
        elif re.match(r'^[a-z]\)', line): # MCQ option
...
def main():
    ...
    # 1. Definitions
    def_path = SOURCE_DIR / "Comp_Definitions.md"
    ...
    # 2. Exam
    exam_path = SOURCE_DIR / "Comp_MockExam.md"
